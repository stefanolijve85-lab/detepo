    ...
    <View style={styles.stepWrap}>
      ...
      <View style={[styles.scanHero, { backgroundColor: colors.surface1 }]}>
        ...
        <Text style={[styles.scanSub, { color: colors.textSecondary }]}>
          {bleOff
            ? "Zet Bluetooth aan in de instellingen van je iPhone."
            : bleUnauth
            ? "Sta Bluetooth-toegang toe in Instellingen → Privacy → Bluetooth."
            : isScanning
            ? `${devices.length} Detepo teller${devices.length !== 1 ? "s" : ""} gevonden`
            : "Houd de teller binnen 2 meter van je iPhone."}
        </Text>
      </View>
      ...
      >
        ...
        >
          {isScanning ? "Bezig met zoeken…" : "Start zoeken"}
        </Text>
      </Pressable>
      ...
      {!isScanning && devices.length === 0 && (
        <View style={[styles.infoCard, { backgroundColor: colors.surface1 }]}>
          ...
          <Text style={[styles.infoText, { color: colors.textSecondary }]}>
            Zet de teller aan en houd hem binnen 2 meter. Tellers zijn zichtbaar als "FP111-xxxxxx" tijdens de koppelprocedure.
          </Text>
        </View>
      )}
    </View>
...
function ConfigureStep({
  ...
  device: BLEDevice;
  onConfirm: (ssid: string, password: string, label: string) => void;
  onBack: () => void;
}) {
  const colors = useColors();
  ...
  const canContinue = ssid.trim().length > 0 && password.trim().length >= 8;
    ...
    <View style={styles.stepWrap}>
      ...
      <View style={[styles.selectedDevice, { backgroundColor: colors.surface1 }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.deviceMeta, { color: colors.textTertiary }]}>Geselecteerde teller</Text>
        </View>
        <Pressable onPress={onBack}>
          <Text style={[styles.changeBtn, { color: colors.cyan }]}>Wijzig</Text>
        </Pressable>
      </View>

      {/* Form */}
      <View style={[styles.formCard, { backgroundColor: colors.surface1 }]}>
        <Text style={[styles.formTitle, { color: colors.foreground }]}>Teller configuratie</Text>

        <View style={styles.field}>
          <Text style={[styles.fieldLabel, { color: colors.textTertiary }]}>NAAM TELLER</Text>
          <TextInput
            value={label}
            onChangeText={setLabel}
            placeholder="bijv. Hoofdingang"
        ...
        <View style={styles.field}>
          <Text style={[styles.fieldLabel, { color: colors.textTertiary }]}>WIFI NETWERK (SSID)</Text>
          <TextInput
            value={ssid}
            onChangeText={setSsid}
            placeholder="Naam van het WiFi-netwerk"
            placeholderTextColor={colors.textTertiary}
            autoCapitalize="none"
            autoCorrect={false}
            style={[styles.input, { backgroundColor: colors.surface2, color: colors.foreground, borderColor: "rgba(255,255,255,0.07)" }]}
        ...
        <View style={styles.field}>
          <Text style={[styles.fieldLabel, { color: colors.textTertiary }]}>WIFI WACHTWOORD</Text>
          <View>
            <TextInput
              value={password}
        ...
        <View style={[styles.serverInfo, { backgroundColor: colors.surface2 }]}>
          ...
          <Text style={[styles.serverText, { color: colors.textTertiary }]}>
            Server: dashboard.detepo.com:8080
          </Text>
        </View>
      </View>

      <Pressable
        onPress={() => canContinue && onConfirm(ssid.trim(), password, label.trim())}
...
function ProgressStep({ step, message, logLines }: { step: string; message: string; logLines: string[] }) {
  ...
  const stages: Array<{ key: string; label: string }> = [
    { key: "connecting",    label: "Verbinding maken" },
    { key: "waitingWifi",   label: "WiFi module wachten" },
    { key: "sendingWifi",   label: "WiFi instellen" },
    { key: "sendingServer", label: "Server instellen" },
  ];
  ...
  return (
    <View style={[styles.stepWrap, styles.centeredStep]}>
      ...
      <Text style={[styles.progressTitle, { color: colors.foreground }]}>
        {step === "connecting" ? "Verbinding maken…" : "Teller configureren…"}
      </Text>
      ...
      {step === "waitingWifi" && (
        <View style={[styles.infoCard, { backgroundColor: colors.surface1, width: "100%" }]}>
          ...
          <Text style={[styles.infoText, { color: colors.textSecondary }]}>
            De teller herstart zijn WiFi-module elke minuut. Wacht geduldig — dit kan tot 90 seconden duren.
          </Text>
        </View>
      )}
    </View>
...
function DoneStep({ deviceName, onClose }: { deviceName: string; onClose: () => void }) {
  ...
  return (
    <View style={[styles.stepWrap, styles.centeredStep]}>
      ...
      <Text style={[styles.progressTitle, { color: colors.foreground }]}>Teller gekoppeld!</Text>
      <Text style={[styles.progressSub, { color: colors.textSecondary }]}>
        {deviceName} is geconfigureerd en verbindt nu met het Detepo dashboard. Dit kan tot 30 seconden duren.
      </Text>
      <Pressable
        onPress={onClose}
      ...
      >
        ...
        <Text style={[styles.primaryBtnText, { color: colors.background }]}>Sluiten</Text>
      </Pressable>
    </View>
  );
}
...
function ErrorStep({ message, onRetry }: { message: string; onRetry: () => void }) {
  ...
  return (
    <View style={[styles.stepWrap, styles.centeredStep]}>
      ...
      <Text style={[styles.progressTitle, { color: colors.foreground }]}>Koppelen mislukt</Text>
      <Text style={[styles.progressSub, { color: colors.textSecondary }]}>{message}</Text>
      <Pressable onPress={onRetry} style={[styles.primaryBtn, { backgroundColor: colors.surface1, marginTop: 12 }]}>
        ...
        <Text style={[styles.primaryBtnText, { color: colors.cyan }]}>Opnieuw proberen</Text>
      </Pressable>
    </View>
  );
}
...
export default function BluetoothScanScreen() {
  ...
  function handleProvision(ssid: string, password: string, label: string) {
    if (!selectedDevice) return;
    connectAndProvision(selectedDevice, ssid, password, label);
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: isWeb ? 20 : insets.top + 8 }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.headerTitle, { color: colors.foreground }]}>Teller verbinden</Text>
          <Text style={[styles.headerSub, { color: colors.textTertiary }]}>Bluetooth LE koppelen</Text>
        </View>
        <View style={[styles.blePill, { backgroundColor: bleState === State.PoweredOn ? "rgba(0,229,160,0.1)" : "rgba(255,59,92,0.1)" }]}>
          ...
          <Text style={[styles.blePillText, { color: bleState === State.PoweredOn ? colors.green : colors.red }]}>
            {bleState === State.PoweredOn ? "Aan" : "Uit"}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.steps}>
        {(["Zoeken", "Configureren", "Koppelen", "Klaar"] as const).map((label, i) => {
          const done =
